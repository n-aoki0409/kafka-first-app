...
public class WindowedStreaming {
    public static void main(final String[] args) {
        ...
        KStream<String, JsonNode> metrics = builder.stream("kafka.metrics", Consumed.with(Serdes.String(), jsonSerde).withTimestampExtractor(new MetricsTimeExtractor()));
        ...
        metrics
            ...
            .groupByKey()
            .windowedBy(TimeWindows.of(Duration.ofSeconds(70000)).advanceBy(Duration.ofSeconds(10000)))
            .aggregate(
            ...
            .flatMapValues(wrap((agg -> {
                ObjectNode o = (ObjectNode)agg;
                o.put("BytesIn_avg_1m", (o.get("last").asLong() - o.get("first").asLong()) / o.get("width").asLong());
                o.remove("timestamp_0");
                return mapper.writeValueAsString(o);
            })))
            .to("kafka.metrics.processed.2", Produced.with(Serdes.String(), Serdes.String()));
